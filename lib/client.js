      ...
      function WhaleApp() {
        ...
        React.useLayoutEffect(() => {
          ...
          if (!staticPos) {
            ...
            let x = window.innerWidth - 216 - 48
            let y = window.innerHeight - 135 * settings.scale - 64
        ...
        function clampStaticPos(x, y) {
          ...
          return {
            x: Math.round(Math.min(Math.max(6, x), Math.max(6, vw - 216 - 6))),
            y: Math.round(Math.min(Math.max(6, y), Math.max(6, vh - 135 * settings.scale - 6)))
          }
        }
        ...
        // 停在上半屏时面板放到鲸鱼下方，否则放在鲸鱼上方。
        const panelStyle = { left: panelLeft == null ? 14 : panelLeft }
        if (isStatic && staticPos) {
          const wh = 135 * settings.scale
          const centerY = staticPos.y + wh / 2
          if (centerY < window.innerHeight / 2) {
            panelStyle.top = Math.round(Math.min(window.innerHeight - 170, Math.max(10, staticPos.y + wh + 14))) + 'px'
          } else {
            panelStyle.bottom = Math.round(Math.max(12, window.innerHeight - staticPos.y + 14)) + 'px'
          }
        } else {
