...
function apply(ctx) {
  ...
  // 并发去重：进行中的检查会被后续调用（含客户端手动「检测更新」）共享，
  // 避免短时间重复请求 registry；完成后清空 in-flight，下次调用重新发起。
  ...
  // ---- Client 轮询接口（webServer HTTP 路由；webServer 已注入，保证路由可注册）----
  {
    ...
    ctx.webServer.register({
      ...
      handler: (req, res) => {
        ...
        } else if (pathname === '/whale/check-update' && req.method === 'POST') {
          // 客户端「检测更新」按钮：手动触发一次版本检查（与定时检查共享 in-flight 去重）。
          // 结果直接返回给发起者，同时更新 latestVersion/updateChecked，
          // 后续 /whale/pull 轮询会同步给所有已打开的页面。
          checkUpdate().then((v) => {
            sendJson(res, 200, {
              ok: true,
              version: VERSION,
        ...
        } else if (pathname === '/whale/check-update') {
          sendJson(res, 405, { error: 'method not allowed' })
        } else {
