...
任务完成时跃出水面。
点击鲸鱼本体可展开/收起会话状态面板（**面板出现位置跟随鲸鱼当前位置，自动考虑左右边界**；
实时会话列表 + 最近动态 + 大小/上下位置/气泡距离/透明度/**主题（深/浅色）**/**提醒方式**设置）。
...
>
> **面板位置跟随鲸鱼**：面板在展开瞬间位于鲸鱼当前位置上方并水平居中，靠近屏幕左右边缘时自动夹紧，
> 不会超出视口；窗口尺寸变化时也会重新夹紧。
