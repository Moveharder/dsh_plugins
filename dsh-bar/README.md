...
| 设置… | 端口 / dsh 路径 / node 路径 / 日志路径；dsh 路径支持「自动检测」「浏览…」与生效状态提示；启动后自动开浏览器、退出时停止服务、登录后自动启动服务 |
...
- **dsh 路径解析优先级**：显式配置 → `~/.local/bin/dsh` → PATH（`command -v`，补齐 GUI 常缺的 `~/.local/bin` 等目录）→ `/opt/homebrew/bin`、`/usr/local/bin` → nvm 各 node 版本的全局 bin（`~/.nvm/versions/node/*/bin/dsh`，支持 NVM_DIR）。留空即自动检测；设置里可一键「自动检测」回填，或用「浏览…」手动选择；菜单与设置里都会显示当前生效的 dsh 路径。dsh 由 dsh-bar 显式用 node 执行，无需把 dsh 加入系统 PATH（PATH 注册只影响你在终端敲 `dsh`）。
